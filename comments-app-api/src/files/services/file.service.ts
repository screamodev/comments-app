...
export class FileService {
    constructor(
        ...
        @InjectQueue('fileOptimizationQueue') private readonly fileOptimizationQueue: Queue,
    ) {}
    ...
    async preUploadFiles(files: Express.Multer.File[]) {
        ...
        for (const file of files) {
            ...
            jobIds.push(job.id);
            this.eventEmitter.emit('file.uploaded', { filename: uniqueFileName, jobId: job.id });
        }
    ...
    async attachFilesToComment(commentId: number, jobIds: string[]) {
        ...
        for (const jobId of jobIds) {
            ...
            const finalFilePath = path.join(__dirname, '../../../db/uploads', originalname);
